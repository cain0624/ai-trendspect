...
(function () {
  ...
  /* ============ SpecularButton 移植（WebGL 镜面高光描边） ============ */
  function initSpecularButtons() {
    ...
    const FRAG = "#version 300 es\nprecision highp float;\n" +
      ...
      "uniform vec3 uLineColor; uniform vec3 uBaseColor; uniform float uIntensity;\n" +
    ...
    function create(btn) {
      if (!btn.querySelector(".btn__label")) {
        btn.innerHTML = '<span class="btn__label">' + btn.innerHTML + "</span>";
      }
      let fx = btn.querySelector(".btn__fx");
      if (!fx) {
      ...
      const U = {};
      ["uCenter", "uHalfSize", "uRadius", "uAngle", "uPx", "uLineColor", "uBaseColor", "uIntensity", "uShineSize", "uShineFade", "uThickness", "uBaseWidth"].forEach((n) => {
      ...
      const inst = {
        btn, canvas, gl, U, dpr,
        radius: parseFloat(cs.getPropertyValue("--sb-radius")) || 0,
        lineColor: hexToRgb(cs.getPropertyValue("--sb-line-color").trim() || "#ffffff"),
        baseColor: hexToRgb(cs.getPropertyValue("--sb-base-color").trim() || "#525252"),
        intensity: 1,
        shineSize: 10,
        shineFade: 40,
        thickness: 1,
        ...
        followMouse: true,
        proximity: 250,
        autoAnimate: !window.matchMedia("(hover: hover)").matches,
        angle: 2.4,
        idleAngle: 2.4,
        bright: 0,
        pointerAngle: null,
      ...
      function resize() {
        ...
        gl.uniform1f(U.uRadius, Math.min(inst.radius, Math.min(w, h) / 2) * dpr);
        gl.uniform1f(U.uIntensity, inst.intensity);
        gl.uniform1f(U.uShineSize, (inst.shineSize * Math.PI) / 180);
        gl.uniform1f(U.uShineFade, (inst.shineFade * Math.PI) / 180);
        gl.uniform1f(U.uThickness, inst.thickness * dpr);
        gl.uniform3fv(U.uLineColor, inst.lineColor);
        gl.uniform3fv(U.uBaseColor, inst.baseColor);
      }
      ...
      inst.update = function (dt) {
        ...
        inst.idleAngle += inst.speed * dt;
        const steer = inst.followMouse && inst.pointerAngle != null && (!inst.autoAnimate || inst.proximityT > 0);
        const target = steer ? inst.pointerAngle : inst.idleAngle;
        const diff = ((target - inst.angle + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
        inst.angle += diff * (1 - Math.exp(-dt * 7));
        const brightTarget = inst.autoAnimate ? 1 : inst.proximityT;
        inst.bright += (brightTarget - inst.bright) * (1 - Math.exp(-dt * 8));
        gl.uniform1f(U.uAngle, inst.angle);
        gl.uniform1f(U.uIntensity, inst.intensity * inst.bright);
    ...
    window.addEventListener("pointermove", (e) => {
      for (const inst of instances) {
        ...
        if (dist === 0) {
          ...
          inst.pointerAngle = Math.atan2(2 / rect.height, -2 / rect.width) + nx * 0.3 + ny * 0.15;
        } else {
          inst.pointerAngle = Math.atan2(cy - e.clientY, e.clientX - cx);
        }
        const t = Math.max(0, 1 - dist / Math.max(inst.proximity, 1));
        inst.proximityT = t * t * (3 - 2 * t);
      }
    }, { passive: true });

    // 视口懒加载：进入视口才创建 WebGL 上下文，离开即销毁（控制上下文数量）
    const io = new IntersectionObserver((entries) => {
      entries.forEach((en) => {
        const btn = en.target;
        const existing = instances.find((i) => i.btn === btn);
    ...
    function frame(now) {
      if (!running) return;
      // 30fps 节流：镜面扫描在低频下依然平滑，显著减少每帧 WebGL 绘制
      if (now - last < 32) {
        raf = requestAnimationFrame(frame);
        return;
      }
      ...
      last = now;
      for (const inst of instances) {
        if (!inst.destroyed) inst.update(dt);
      }
      raf = requestAnimationFrame(frame);
    }
    document.addEventListener("visibilitychange", () => {
